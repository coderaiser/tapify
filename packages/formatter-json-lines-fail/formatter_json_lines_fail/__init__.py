...
class _JsonLinesFail:
    """Formatter instance with isolated state (createFormatter protocol).

    Like formatter-json-lines, but successful ``test_end`` lines are
    suppressed (SUPERTAPE_JSON_LINES_FAIL behaviour) — only fail lines and
    the final summary line are emitted.
    """
    ...
    def test_end(self, *, count=0, total=0, failed=0, test="", **_) -> None:
        del count, total, failed, test
        return None
...
def create_formatter(**_):
    ...
    return _JsonLinesFail()


_default = _JsonLinesFail()
